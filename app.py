...
st.markdown(
    ...
    The CSV is loaded from `data/utah_housing.csv`. Run the data acquisition notebook first if
...
try:
    ...
    st.error(
        f"`{DATA_PATH}` not found. Run `data_acquisition.ipynb` first to generate it."
    )
    st.stop()
